...
void AShooterCharacter::UpdatePawnMeshes()
{
	...
	Mesh1P->MeshComponentUpdateFlag = !firstPerson ? EMeshComponentUpdateFlag::OnlyTickPoseWhenRendered : EMeshComponentUpdateFlag::AlwaysTickPoseAndRefreshBones;
	Mesh1P->SetOwnerNoSee(!firstPerson);

	GetMesh()->MeshComponentUpdateFlag = firstPerson ? EMeshComponentUpdateFlag::OnlyTickPoseWhenRendered : EMeshComponentUpdateFlag::AlwaysTickPoseAndRefreshBones;
	GetMesh()->SetOwnerNoSee(firstPerson);
}
...
void AShooterCharacter::GetPositions(TArray<FVector>& Locations) const {
	USkeletalMeshComponent* mesh = GetMesh();
	if (!mesh) {
		GEngine->AddOnScreenDebugMessage(-1, 5, FColor::White, TEXT("Mesh Null"));
		return;
	}
	...
	bool DoVelocityCorrection = true && MovementComp;
	//Pawn Velocity Correction
